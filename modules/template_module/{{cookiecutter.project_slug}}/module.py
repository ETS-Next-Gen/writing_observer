...
EXECUTION_DAG = {
    'execution_dag': {
        ...
        'student_keys': q.keys('{{ cookiecutter.project_slug }}.{{ cookiecutter.reducer }}', STUDENTS=q.variable('roster'), STUDENTS_path='user_id'),
        # q.select handles fetching items from redis based on a list of keys
        '{{ cookiecutter.reducer }}s': q.select(q.variable('student_keys'), fields={'count': 'count'}),
        # q.join will combine two lists of dictionaries based on a key_path
        '{{ cookiecutter.reducer }}_join_roster': q.join(LEFT=q.variable("{{ cookiecutter.reducer }}s"), RIGHT=q.variable("roster"), LEFT_ON='provenance.provenance.value.user_id', RIGHT_ON='user_id'),
    },
    'exports': {
        'student_{{ cookiecutter.reducer }}s': {
            'returns': '{{ cookiecutter.reducer }}_join_roster',
...
COURSE_DASHBOARDS = [{
    'name': NAME,
    'url': "/{{ cookiecutter.project_slug }}/dash/lo-example",
    "icon": {
        "type": "fas",
        "icon": "fa-play-circle"
    }
