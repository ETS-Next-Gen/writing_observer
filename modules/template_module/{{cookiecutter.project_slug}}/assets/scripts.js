...
window.dash_clientside.{{ cookiecutter.project_slug }} = {
    ...
    if (wsReadyState.readyState === 1) {
      ...
      const outgoingMessage = {
        {{ cookiecutter.project_slug }}_query: {
          ...
          target_exports: ['student_{{ cookiecutter.reducer }}s'],
  ...
  receiveWSMessage: async function (incomingMessage) {
    ...
    //  called the target export `student_{{ cookiecutter.reducer }}s`, i.e. the named
    // export. Below, we need to call `{{ cookiecutter.reducer }}_join_roster`, i.e. the name
