...
window.dash_clientside.lo_toy_sba = {
  /**
  ...
  receiveWSMessage: async function (incomingMessage) {
    ...
    //  called the target export `student_event_counter_export`, i.e. the named
    // export. Below, we need to call `lo_toy_sba_join_roster`, i.e. the name
    // of the node. This ought to be cleaned up in the communication protocl.
    const messageData = JSON.parse(incomingMessage.data).lo_toy_sba_query.student_event_counter_join_roster || [];
